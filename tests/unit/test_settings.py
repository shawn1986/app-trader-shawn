...
def test_load_settings_accepts_scan_input_overrides(tmp_path: Path) -> None:
    config_dir = tmp_path / "config"
    config_dir.mkdir()
    write_config(
        config_dir,
    ...
    assert settings.scan_inputs.min_dte == 5
    assert settings.scan_inputs.max_dte == 35
    assert settings.scan_inputs.strike_window_pct == 0.25
    assert settings.scan_inputs.fallback_strike_count == 12
    assert settings.scan_inputs.max_expiries == 3
